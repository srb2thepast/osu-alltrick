...
{
    public partial class BarGraphFlow : FillFlowContainer<Bar>
    {
        /// <summary>
        ...
        public new BarDirection Direction
        {
            ...
            {
                direction = value;
                base.Direction = direction.HasFlagFast(BarDirection.LeftToRight) ? FillDirection.Vertical : FillDirection.Horizontal;

                foreach (var bar in Children)
                {
                    bar.Size = direction.HasFlagFast(BarDirection.LeftToRight) ? new Vector2(1, 1.0f / Children.Count) : new Vector2(1.0f / Children.Count, 1);
            ...
            set
            {
                ...
                foreach (var bar in value.Select((length, index) => new { Value = length, Bar = bars.Count > index ? bars[index] : null }))
                {
                    ...
                    if (bar.Bar != null)
                    {
                        ...
                        bar.Bar.Size = direction.HasFlagFast(BarDirection.LeftToRight) ? new Vector2(1, size) : new Vector2(size, 1);
                    }
                    else
                    {
                        Add(new Bar
                        {
                            ...
                            Size = direction.HasFlagFast(BarDirection.LeftToRight) ? new Vector2(1, size) : new Vector2(size, 1),
                            Length = length,
                            Direction = Direction,
                        });
                    }
